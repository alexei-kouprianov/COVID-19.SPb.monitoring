...
# Preparing tickmarks;
years.tickmarks <- strptime((seq(as.Date("2006-01-01"), length=19, by="1 year") - 1), "%Y-%m-%d")
...
.report.SPb.deaths.cum <- sum(spb.united$DEATHS.sk, na.rm = TRUE)
.report.SPb.deaths.0107 <- sum(tail(spb.united$DEATHS.sk, 7), na.rm = TRUE)
.report.SPb.deaths.0814 <- sum(tail(spb.united$DEATHS.sk, 14)[1:7], na.rm = TRUE)
.report.SPb.confirmed.0107 <- sum(tail(spb.united$CONFIRMED.sk, 7), na.rm = TRUE)
.report.SPb.confirmed.0814 <- sum(tail(spb.united$CONFIRMED.sk, 14)[1:7], na.rm = TRUE)
