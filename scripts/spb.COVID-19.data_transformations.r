...
if((.report.SPb.confirmed.0107 / .report.SPb.confirmed.0814 < 1.02 |
	...
	(.report.SPb.deaths.0107 / .report.SPb.deaths.0814 < 1.02 |
		...
		.report.SPb.phase.modifier <- "Скорость изменений критически замедлилась, возможно, вскоре направление тренда сменится на противоположное."
	} else{
...
spb.adj$DEATHS.exc_to_2019 <- c(
	...
	c(2000, 3700, 11000), # Kobak's extrapolations for Estonia, Latvia, Lithuania (updated manually on the monthly basis), see https://github.com/dkobak/excess-mortality for the want of a better estimate;
...
spb.excessive_deaths.sk.tck <- timeline.tickmarks[7:23]
